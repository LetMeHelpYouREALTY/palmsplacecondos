...
- **LocalBusiness + listing agent:** the primary listing agent node is typed as both `RealEstateAgent` and `LocalBusiness` with a single stable `@id` (`#dr-jan-duffy`). Phone, email, address, hours, and optional office `geo` / `hasMap` must match **Google Business Profile** and visible NAP—do not add weekend hours until GBP is updated.
